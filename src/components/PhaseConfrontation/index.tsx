...
const PhaseConfrontation = ({selection1, gols1, flag1, selection2, gols2, flag2, color, colorBorderMatchNumber, originImage, numberConfrontation, roundNumber}:PhaseConfrotationProps)=>{
    return(
        ...
        <div className={`lg:w-1/3 w-4/5 flex justify-center items-center bg-card py-3 px-2 lg:m-5 m-2 shadow-md rounded-lg uppercase relative hover:shadow-2xl`}>
            <span className={`absolute -top-2 bg-card flex justify-center items-center text-xs font-semibold w-6 h-6 ${colorBorderMatchNumber} border-2 rounded-b-full ${color}`}>{numberConfrontation}</span>
            <div  className="flex flex-1 justify-center items-center flex-col">
                <Image className="bg-[#D9D9D9] shadow-sm rounded-full lg:w-[50px] lg:h-[50px] w-10 h-10" src={`/assets/${originImage}/${flag1}.svg`} width={50} height={50} alt=""/>
