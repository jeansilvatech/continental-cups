...
interface PhaseConfrotationProps {
    ...
    flag2: string;
    color: string;
    colorBorderMatchNumber: "euro"| "america";
...
const PhaseConfrontation = ({selection1, gols1, flag1, selection2, gols2, flag2, color, colorBorderMatchNumber, originImage, numberConfrontation, roundNumber}:PhaseConfrotationProps)=>{
    return(
        ...
        <div className={`lg:w-1/3 w-4/5 flex justify-center items-center bg-[#E9E9E9] py-3 px-2 lg:m-5 m-2 shadow-md rounded-lg uppercase relative`}>
            <span className={`absolute -top-2 bg-[#E9E9E9] flex justify-center items-center text-xs font-semibold w-5 h-5 border-${colorBorderMatchNumber} border-[1px] rounded-full ${color}`}>{numberConfrontation}</span>
