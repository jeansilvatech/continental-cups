                        ...
                        <div key={index} className="bg-america1 lg:w-[68%] w-full m-2 flex justify-evenly items-center flex-wrap shadow-md rounded-lg animate-enter">
                            <TitleCardPhase title={item.phase} color="america"/>
                            {
                                item.matches.map((match:any, index:number)=>(
                                    <PhaseConfrontation roundNumber={match.round} numberConfrontation={match.decision?match.decision:index+1} key={index} selection1={match.name1} gols1={match.gols1} flag1={match.flagUrl1} selection2={match.name2} gols2={match.gols2} flag2={match.flagUrl2} color="text-america" colorBorderMatchNumber="border-america" originImage="america"/>
                                ))
                            }
